...
import { View, Image, StyleSheet, Text, TouchableOpacity, TextInput, SafeAreaView, ImageBackground, Alert } from 'react-native'
...
export default class CreateOpenHouse extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            date: '',
            dateForAPI: new Date(),
            yesOrNo: [{ id: 0, name: 'Yes' }, { id: 1, name: 'No' }],
            BasementValues: [{ id: 0, name: 'Any' }, { id: 1, name: 'Has Basement' }, { id: 2, name: 'Finished Basement' }, { id: 2, name: 'Unfinished Basement' }, { id: 2, name: 'Walkout Basement' }, { id: 3, name: 'No Basement' }],
            otherAmenitiesArray: [{ id: 0, name: 'Other Anemities' }, { id: 1, name: 'Must Have Ac' }, { id: 2, name: 'Must have pool' }, { id: 2, name: 'Waterfront' }],
            otherViewsArray: [{ id: 0, name: 'Views' }, { id: 1, name: 'City' }, { id: 2, name: 'Mountain' }, { id: 2, name: 'Park' }, { id: 2, name: 'water' }],
            openHouseTitle: "",
            city: "",
            cityList: [],
            openHouseTypeList: [],
            ...
            squareFeetList: ['50 sq ft', '100 sq ft', '1000 sq ft', '2000 sq ft', '3000 sq ft', '4000 sq ft', '5000 sq ft',],
            showSquareFeet: false,
            squareFeetSize: 'Square Feet',
            selectedSquareFeet: 2,
            showMinPlotSize: false,
            minimumPlotSize: 'No Min',
            minimumPlotSizeList: ['0.25 sq ft', '0.5 sq ft', '1 sq ft', '2 sq ft', '5 sq ft', '10 sq ft', '50 sq ft', '100 sq ft'],
            selectedMinimumPlotSize: 2,

            showMaxPlotSize: false,
            maximumPlotSize: 'No Max',
            maximumPlotSizeList: ['1000 sq ft', '2000 sq ft', '3000 sq ft', '4000 sq ft', '5000 sq ft',],
    ...
    componentDidMount() {
        ...
        if (this.state.isEdit) {
            ...
            this.setState({
                ...
                floorPlan: item.floorplan_image_1 ? item.floorplan_image_1 : item.floorplan_image_2 ? item.floorplan_image_2 : item.floorplan_image_3 ? item.floorplan_image_3 : item.floorplan_image_4,
    ...
    onPickerSquareFeetSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ squareFeetSize: `${this.state.squareFeetList[this.state.selectedSquareFeet]}`, squareFeet: this.state.squareFeetList[this.state.selectedSquareFeet] })
        });
    };
    onPickerMinimumPlotSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ minimumPlotSize: `${this.state.minimumPlotSizeList[this.state.selectedMinimumPlotSize]}`, plotSize: this.state.minimumPlotSizeList[this.state.selectedMinimumPlotSize] })
        });
    };
    onPickerYearSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ year: `${this.state.yearList[this.state.selectedYear]}`, yearBuilt: `${this.state.yearList[this.state.selectedYear]}` })
        });
    };
    onPickerAmenitiesSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ otherAmenitiesText: `${this.state.otherAmenitiesList[this.state.selectedAmenities]}`, otherAmenities: `${this.state.otherAmenitiesList[this.state.selectedAmenities]}` })
        });
    };
    onPickerViewsSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ otherViewsText: `${this.state.otherViewsList[this.state.selectedViews]}`, propertyViews: `${this.state.otherViewsList[this.state.selectedViews]}` })
        });
    };
    onPickerMaximumPlotSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ maximumPlotSize: `${this.state.maximumPlotSizeList[this.state.selectedMaximumPlotSize]}`, maxPlotSize: this.state.maximumPlotSizeList[this.state.selectedMaximumPlotSize] })
        });
    };
    onPickerCitySelect(index) {
        ...
        this.setState({
            ...
            this.setState({ cityName: this.state.cityList[this.state.selectedCityItem].name, cityId: `${this.state.cityList[this.state.selectedCityItem].id}`, })
        });
    };
    onPickerHouseTypeSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ HouseTypeName: this.state.openHouseTypeList[this.state.selectedHouseType].name, openHouseType: `${this.state.openHouseTypeList[this.state.selectedHouseType].id}`, })
        });
    };
    ...
    onPickerRepeatTypeSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ RepeatType: this.state.repeatTypeList[this.state.selectedRepeatType].name, repeatType: `${this.state.repeatTypeList[this.state.selectedHouseType].id}`, })
        });
    };
    onPickerBedroomSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ bedRoomName: this.state.bedroomList[this.state.selectedBedRoom].name, bedrooms: `${this.state.bedroomList[this.state.selectedBedRoom].id}`, })
        });
    };
    onPickerBathroomSelect(index) {
        ...
        this.setState({
            ...
            this.setState({ bathRoomName: this.state.bathroomList[this.state.selectedBathRoom].name, bathrooms: `${this.state.bathroomList[this.state.selectedBathRoom].id}`, })
        });
    };
    ...
    getOpenHouseType = () => {
        ...
        if (userData) {
            this.setState({ userId: userData.id }, () => {
                console.log('check for user data in the create house===.', this.state.userId);
            })
        }
        else {
        ...
        fetch(API.GET_TAXONOMIES, {
            ...
            if (response.body) {
                let temp = response.body.open_house_type, index;
                let city = response.body.city, cityindex;
                ...
                if (this.state.isEdit) {
                    for (let i = 0; i < Bedroom.length; i++) {
                        ...
                        if (Bedroom[i].name == this.state.bedrooms) {
                            this.setState({ selectedBedRoom: i, bedRoomName: response.body.bedroom[i].name, bedrooms: response.body.bedroom[i].id })
                        }
                    };
                    for (let i = 0; i < bathroom.length; i++) {
                        if (bathroom[i].name == this.state.bathrooms) {
                            console.log('hhgjhjhkj', i)
                            this.setState({ selectedBathRoom: i, bathRoomName: response.body.bathroom[i].name, bathrooms: response.body.bathroom[i].id })
                        }
                    };
                    for (let i = 0; i < temp.length; i++) {
                        if (temp[i].name == this.state.openHouseType) {
                            this.setState({ selectedHouseType: i, HouseTypeName: temp[i].name, openHouseType: temp[i].id })
                        }
                    };
                    for (let i = 0; i < city.length; i++) {
                        if (city[i].name == this.state.city) {
                            this.setState({ selectedCityItem: i, cityName: city[i].name, cityId: city[i].id })
                            // cityindex = i
                        }
                    };
                    ...
                    for (let i = 0; i < this.state.minimumPlotSizeList.length; i++) {
                        if (this.state.minimumPlotSizeList[i].includes(this.state.plotSize)) {
                            this.setState({ selectedMinimumPlotSize: i, minimumPlotSize: this.state.minimumPlotSizeList[i], plotSize: this.state.minimumPlotSizeList[i] })
                        }
                    }
                    for (let i = 0; i < this.state.maximumPlotSizeList.length; i++) {
                        if (this.state.maximumPlotSizeList[i].includes(this.state.maxPlotSize)) {
                            this.setState({ selectedMaximumPlotSize: i, maximumPlotSize: this.state.maximumPlotSizeList[i], maxPlotSize: this.state.maximumPlotSizeList[i] })
                        }
                    }
                    for (let i = 0; i < this.state.itemList.length; i++) {
                        if (this.state.itemList[i] == this.state.basement) {
                            this.setState({ selectedItem: i, basement: this.state.itemList[i], })
                        }
                    }
                    ...
                    for (let i = 0; i < this.state.otherAmenitiesList.length; i++) {
                        if (this.state.otherAmenitiesList[i] == this.state.otherAmenities) {
                            this.setState({ selectedAmenities: i, otherAmenitiesText: this.state.otherAmenitiesList[i], otherAmenities: this.state.otherAmenitiesList[i] })
                        }
                    }
                    for (let i = 0; i < this.state.otherViewsList.length; i++) {
                        if (this.state.otherViewsList[i] == this.state.propertyViews) {
                            this.setState({ selectedViews: i, otherViewsText: this.state.otherViewsList[i], propertyViews: this.state.otherViewsList[i] })
                        }
                    }
                }
                this.setState({
    ...
    checkFields = () => {
        const {
            profileImage, profileImageTwo, profileImageThree, profileImageFour, profileImageFive,
            openHouseTitle, streetAddress, city, state, zip, date, dateForAPI, openHouseType, propertyDescription,
            bedrooms, bathrooms, propertyPrice, squareFeet, plotSize, basement, yearBuilt, otherAmenities,
            propertyViews, schoolDistrict, keywords, floorPlan, maxPlotSize, cityName, closingDate, startTime, endTime
        } = this.state;
        let splitStartTime;
        let splitEndTime;
        ...
        // }
            ...
            Alert.alert(null, 'Open house start date should not be greater than open house end date')
            return false
        }
         if (startTime == '') {
    ...
    createOpenHouseApi = () => {
        ...
        const {
            sourceForAPI, sourceForAPITwo, sourceForAPIThree, sourceForAPIFour, sourceForAPIFive,
            sourceForAPIFloorPlan, sourceForAPIVirtualTour, openHouseTitle, streetAddress, city, state, zip,
            date, dateForAPI, openHouseType, propertyDescription, bedrooms, bathrooms, propertyPrice, squareFeet, plotSize,
            basement, yearBuilt, otherAmenities, propertyViews, schoolDistrict, keywords, lat, long, userId, cityId, postId, maxPlotSize, closingDate, RepeatType, startTime, endTime
        } = this.state
        ...
        axios.post(API.INSERT_UPDATE_OPEN_HOUSE,
            requestBody, {
            headers: {
                "Accept": "application/json",
            ...
            this.setState({ loading: false, loadingOnUpload: false, percentCompleted: 0 })
    ...
    handleChoosePhoto() {
        ...
        ImagePicker.showImagePicker(options, (response) => {
            if (response.didCancel) {
                ...
                const source = { uri: response.uri, name: moment().format('x') + '.jpeg', type: 'image/jpeg' };
                ...
                <View style={{ width: "100%" }}>
                    <Calendar
                        ...
                        onDayPress={(date) => {
                            ...
                            this.setState({ isCalender: false, date: moment(date.timestamp).format('MM-DD-YYYY'), dateForAPI: date.timestamp })
                        }}
                    />
                </View>
            </View>
    ...
    ClosingSetDate = (text) => {
        ...
        if (/^[0-9-]*$/.test(text)) {
            ...
            if (closingDate.length < text.length && (text.length == 2 || text.length == 5)) {
                text += '-'
                this.setState({ closingDate: text })
            } else {
                this.setState({ closingDate: text })
    ...
    render() {
        ...
        const {
            loadingOnUpload, percentCompleted,appThemeMode,
            profileImage, profileImageTwo, profileImageThree, profileImageFour, profileImageFive,
            city, cityList, openHouseTypeList, openHouseType, bedroomList, bedrooms, bathroomList, bathrooms, yesOrNo,
            basement, otherAmenities, propertyViews, floorPlan, virtualTour, BasementValues, otherAmenitiesArray, otherViewsArray, cityId, showCityList, selectedCityItem,
            showOpenHouseType, selectedHouseType, HouseTypeName, selectedRepeatType, repeatTypeList
        } = this.state;
                ...
                >
                    <View style={{ flex: 1, margin: 10, }}>
                        ...
                        <TextInput
                            ...
                            onChangeText={(text) => { this.setState({ openHouseTitle:  text }) }}
                        ...
                        <View>
                            <TextInput
                                placeholderTextColor='#707071'
                                // onChangeText={(text) => { this.setState({ date: text }) }}
                                onChangeText={(text) => { this.ClosingSetDate(text) }}
                        ...
                        <View>
                            <TextInput
                                placeholderTextColor='#707071'
                                // onChangeText={(text) => { this.setState({ date: text }) }}
                                onChangeText={(text) => { this.startTimeSet(text) }}
                        ...
                        <View>
                            <TextInput
                                placeholderTextColor='#707071'
                                // onChangeText={(text) => { this.setState({ date: text }) }}
                                onChangeText={(text) => { this.endTimeSet(text) }}
                        ...
                        <View style={style.dropInputCont}>
                            <TouchableOpacity onPress={() => this.setState({ showRepeatType: !this.state.showRepeatType })} style={{ flex: 1 }}>
                                <Text style={{ paddingLeft: 10,color:appThemeMode==='light' ? Colors.black :Colors.white }}>{this.state.RepeatType}</Text>
                            </TouchableOpacity>
                            <Image source={require('./../../../assets/images/drop_down.png')}
                                resizeMode={'contain'}
                                style={style.dropImg}>
                        ...
                        {this.state.showRepeatType &&
                            <Picker style={{ width: 200, height: 180, alignSelf: "center" }}
                                lineColor="#000000" //to set top and bottom line color (Without gradients)
                                // lineGradientColorFrom="#008000" //to set top and bottom starting gradient line color
                                // lineGradientColorTo="#FF5733" //to set top and bottom ending gradient
                        ...
                        <View style={style.dropInputCont}>
                            <TouchableOpacity onPress={() => this.setState({ showOpenHouseType: !this.state.showOpenHouseType })} style={{ flex: 1 }}>
                                <Text style={{ paddingLeft: 10,color:appThemeMode==='light' ? Colors.black :Colors.white }}>{this.state.HouseTypeName}</Text>
                            </TouchableOpacity>
                            <Image source={require('./../../../assets/images/drop_down.png')}
                                resizeMode={'contain'}
                                style={style.dropImg}>
                        ...
                        <TextInput
                            placeholderTextColor='#707071'
                            onChangeText={(text) => { this.setState({ propertyDescription: text }) }}
                        ...
                        />
                        <View style={style.dropInputCont}>
                            <TouchableOpacity onPress={() => this.setState({ showBedRoomList: !this.state.showBedRoomList })} style={{ flex: 1 }}>
                                <Text style={{ paddingLeft: 10,color:appThemeMode==='light' ? Colors.black :Colors.white }}>{this.state.bedRoomName}</Text>
                            </TouchableOpacity>
                            <Image source={require('./../../../assets/images/drop_down.png')}
                                resizeMode={'contain'}
                                style={style.dropImg}>
                        ...
                        <View style={style.dropInputCont}>
                            <TouchableOpacity onPress={() => this.setState({ showBathRoomList: !this.state.showBathRoomList })} style={{ flex: 1 }}>
                                <Text style={{ paddingLeft: 10 ,color:appThemeMode==='light' ? Colors.black :Colors.white}}>{this.state.bathRoomName}</Text>
                            </TouchableOpacity>
                            <Image source={require('./../../../assets/images/drop_down.png')}
                                resizeMode={'contain'}
                                style={style.dropImg}>
                        ...
                        <CurrencyInput
                            ...
                            placeholderTextColor='#707071'
                            value={this.state.propertyPrice}
                            onChangeValue={(text) => { this.setState({ propertyPrice: text }) }}
      prefix="$"
      delimiter=","
      separator="."
      precision={0}
    ...
    />
                        ...
                        {this.state.showMinPlotSize &&
                            <Picker style={{ width: 200, height: 180, }}
                                lineColor="#000000" //to set top and bottom line color (Without gradients)
                                // lineGradientColorFrom="#008000" //to set top and bottom starting gradient line color
                                // lineGradientColorTo="#FF5733" //to set top and bottom ending gradient
                        ...
                        {this.state.showMaxPlotSize &&
                            <Picker style={{ width: 200, height: 180, alignSelf: 'flex-end' }}
                                lineColor="#000000" //to set top and bottom line color (Without gradients)
                                // lineGradientColorFrom="#008000" //to set top and bottom starting gradient line color
                                // lineGradientColorTo="#FF5733" //to set top and bottom ending gradient
                        ...
                        <View style={style.dropInputCont}>
                            <TouchableOpacity onPress={() => this.setState({ showModal: !this.state.showModal })} style={{ flex: 1 }}>
                                <Text style={{ paddingLeft: 10,color:appThemeMode==='light' ? Colors.black :Colors.white }}>{this.state.basement}</Text>
                            </TouchableOpacity>
                            <Image source={require('./../../../assets/images/drop_down.png')}
                                resizeMode={'contain'}
                                style={style.dropImg}>
                        ...
                        <TextInput
                            placeholderTextColor='#707071'
                            onChangeText={(text) => { this.setState({ yearBuilt: text }) }}
                        ...
                        <View style={style.dropInputCont}>
                            <TouchableOpacity onPress={() => this.setState({ showAmenities: !this.state.showAmenities })} style={{ flex: 1, justifyContent: 'center' }}>
                                <Text style={{ paddingLeft: 10 }}>{this.state.otherAmenitiesText}</Text>
                            </TouchableOpacity>
                            <Image source={require('./../../../assets/images/drop_down.png')}
                                resizeMode={'contain'}
                                style={style.dropImg}>
                        ...
                        <View style={style.dropInputCont}>
                            <TouchableOpacity onPress={() => this.setState({ otherViews: !this.state.otherViews })} style={{ flex: 1 }}>
                                <Text style={{ paddingLeft: 10 }}>{this.state.otherViewsText}</Text>
                            </TouchableOpacity>
                            <Image source={require('./../../../assets/images/drop_down.png')}
                                resizeMode={'contain'}
                                style={style.dropImg}>
                        ...
                        <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                            ...
                            <TextInput
                                placeholderTextColor='#707071'
                                onChangeText={(text) => { this.setState({ schoolDistrict: text }) }}
                        ...
                        <View style={{ marginTop: 10 }}>
                            <ImageBackground
                                ...
                                <TouchableOpacity
                                    onPress={() => {
                                        this.setState({ imageClick: '7', show9: false }, () => { this.forceUpdate() })
                                        this.selectImageButtonPress()
                                    }}
                                    style={{ flexDirection: 'row' }}>
                                    <Image
                ...
                {loadingOnUpload &&
                    <SafeAreaView style={{ width: '100%', height: 5, bottom: 0, position: 'absolute', justifyContent: 'center' }}>
                        <View style={{ width: (percentCompleted + '%'), height: 5, backgroundColor: '#005271' }}></View>
                    </SafeAreaView>
                }
            </View >
        )
