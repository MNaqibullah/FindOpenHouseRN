...
export default class search extends Component {
    ...
    componentDidMount = async () => {
        ...
        if (platformConst === 'ios') {
            checkConst = PERMISSIONS.IOS.ACCESS_FINE_LOCATION
        } else {
            checkConst = PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION
        } 
           
        Permissions.request(PERMISSIONS.IOS.ACCESS_FINE_LOCATION).then(result => {
...
// alert("sdfdsgsd")
    ...
    getPropertyByLocationApi = () => {
        ...
        if (this.state.isFromDraw) {
            return
        }
        console.log('check state===>', this.state.isFromFilter)
        ...
        fetch(API.GET_PROPERTY_BY_LOCATION, {
            ...
            if (response.status === "200") {
                ...
                this.setState({ propertyList: array, BackUpArray: array })
                ...
                {this.state.showMap && this.state.initialRegionNearLocation.latitude !== null ?
                    <View>
                        ...
                        <View style={{ position: 'absolute', right: 15, top: 20 }}>
                            ...
                            {/* <TouchableOpacity onPress={() => {
                                this.props.navigation.navigate("MapCircle", {
                                    radius: this.radiusValue,
                                    updateIsFromDraw: this.updateIsFromDraw,
                                    initialRegion: this.state.initialRegionNearLocation
                                })
                            }} style={style.circle}>
                                <Image
